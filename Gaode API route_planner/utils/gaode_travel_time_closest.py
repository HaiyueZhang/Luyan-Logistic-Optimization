...
def get_straight_line_distance(point1, point2):
    """Return the straight-line distance between two points."""
    lat1, lon1 = [float(i) for i in point1.split(",")]
    lat2, lon2 = [float(i) for i in point2.split(",")]
    return distance.euclidean((lat1, lon1), (lat2, lon2))

MAX_DISTANCE_CHECKS = 500 

def get_closest_points(locations, closest_num=1):
    """Return a list of index pairs where each pair is (index, closest_num-th closest index)."""
    pairs = []
    # Sort locations by latitude
    sorted_indexes = sorted(range(len(locations)), key=lambda i: float(locations[i].split(",")[0]))
    for i in range(len(locations)):
        # Limit the number of points to check distance against
        start_idx = max(0, sorted_indexes.index(i) - MAX_DISTANCE_CHECKS // 2)
        end_idx = min(len(locations), sorted_indexes.index(i) + MAX_DISTANCE_CHECKS // 2)
        limited_sorted_indexes = sorted_indexes[start_idx:end_idx]

        distances = [(j, get_straight_line_distance(locations[i], locations[j])) for j in limited_sorted_indexes if i != j]
        distances.sort(key=lambda x: x[1])
        if closest_num <= len(distances):  # Ensure there is a closest_num-th closest point
            closest_index = distances[closest_num-1][0]
...
def main():
    ...
    CHECKPOINT_CSV = os.path.join(output_directory, f"progress_checkpoint_closest{input_filename_without_ext}.csv")
    
    locations = pd.read_csv(input_filepath)
    locations = locations.dropna(subset=['location']).reset_index(drop=True)  # Add this line
    ...
    if A_EQUAL_B: 
        origin_destination_idx = get_closest_points(locations_list, closest_num=closest_num)
    else:
        pairs_A = get_closest_points(locations_list, closest_num=closest_num)
        pairs_B = [(b,a) for (a,b) in get_closest_points(locations_list, closest_num=closest_num)]
        origin_destination_idx = pairs_A + pairs_B
    ...
    origin_destination_idx = [pair for pair in origin_destination_idx if pair not in processed_pairs]
    ...
    final_csv_name = f"{Path(__file__).parent}/../gaode/({closest_num})_closest_location_{input_filename_without_ext}_{now}.csv"
