...
START_INDEX = 1   # Example value
END_INDEX = 10    # Example value
...
def main():
    ...
    CHECKPOINT_CSV = os.path.join(output_directory, f"progress_checkpoint_interval{input_filename_without_ext}.csv")
    ...
    start_origin = locations.index[START_INDEX]
    end_origin = locations.index[END_INDEX]
    origin_destination_idx = [(origin, destination) for origin in range(start_origin, end_origin + 1) for destination in locations.index if destination > origin]
    ...
    final_csv_name = f"{Path(__file__).parent}/../gaode/[{START_INDEX} - {END_INDEX}]trip_details_{input_filename_without_ext}_{now}.csv"
