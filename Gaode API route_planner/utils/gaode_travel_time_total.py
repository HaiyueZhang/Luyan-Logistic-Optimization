...
def main():
    ...
    input_filepath = f"{Path(__file__).parent}/../gaode/2023-07-18_Xiamen.csv"
    ...
    if A_EQUAL_B: 
        origin_destination_idx = list(combinations(range(n_locations), 2))
    else:
        origin_destination_idx = list(combinations(range(n_locations), 2)) + list(combinations(range(n_locations-1, -1, -1), 2))
    ...
    final_csv_name = f"{Path(__file__).parent}/../gaode/trip_details_{input_filename_without_ext}_{now}.csv"
